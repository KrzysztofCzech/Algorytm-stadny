...
from communication_operators import crossover, mutation
...
class CommuncationWithMutation(CommunicationType):
    ...
    def comunnicate(self, agent1, agent2):
        ...
        best_fitness11 = agent2.Island.algorithm.solutions[0].objectives[0]
        best_fitness22 = min([sol.objectives[0] for sol in agent2.Island.algorithm.solutions])
        new_population = agent2.transfer_data(agent1.name)
        solutions_old = agent1.get_solutions()
        mean_old = mean_of_solutions(solutions_old)
        mean_new = mean_of_solutions(new_population)
        if mean_new < mean_old:
            ...
            best_fitness3 = agent2.Island.algorithm.solutions[0].objectives[0]
